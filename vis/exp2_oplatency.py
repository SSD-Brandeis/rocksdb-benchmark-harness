...
PLOTS_DIR = PROJECT_ROOT_DIR / "plots.local"
...
OPERATIONS = frozenset(["insert", "update", "pointquery"])


def load_op_latency(system: str):
    ops = {"insert": [], "pointquery": [], "update": []}
    for file in EXPERIMENT_DIR.glob(f"op-latency.{system}.*.json"):
        with open(file) as f:
            arr = json.load(f)
        for d in arr:
...
def plot_system(system: str):
    ...
    order = [op for op in OPERATIONS if op in data]
    if not order:
    ...
    for i, op in enumerate(order, start=1):
        ...
        ax.scatter(
            ...
            color="tab:blue",
            linewidths=2,
            label="p95" if first else "_nolegend_",
        )
        first = False
    ...
    for i, op in enumerate(order, start=1):
        ...
        ax.scatter(
            ...
            color="tab:orange",
            linewidths=2,
            label="p60" if first else "_nolegend_",
        )
        # first = False
    # ---- draw p70
    # first = True
    for i, op in enumerate(order, start=1):
        ...
        ax.scatter(
            ...
            color="tab:red",
            linewidths=2,
            label="p70" if first else "_nolegend_",
        )
    # ---- draw p99
    # first = True
    # for i, op in enumerate(order, start=1):
    ...
    out = PLOTS_DIR / f"op-latency_box_{system}.pdf"
